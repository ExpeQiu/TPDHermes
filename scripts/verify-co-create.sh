...
# 1. 创建测试项目
PROJECT=$(curl -sfLL "${HEADERS[@]}" -X POST "${API_BASE}/projects/" \
...
# 2. 统一文件列表
FILES=$(curl -sfL "${HEADERS[@]}" "${API_BASE}/projects/${PROJECT_ID}/files")
...
SESSION=$(curl -sfLL "${HEADERS[@]}" -X POST "${API_BASE}/chat/sessions" \
...
PATCHED=$(curl -sfL "${HEADERS[@]}" -X PATCH "${API_BASE}/chat/sessions/${SESSION_ID}" \
...
APPLY=$(curl -sfL "${HEADERS[@]}" -X POST "${API_BASE}/projects/${PROJECT_ID}/file-actions/apply" \
...
# 5. 文件详情
DETAIL=$(curl -sfL "${HEADERS[@]}" "${API_BASE}/projects/${PROJECT_ID}/files/${FILE_ID}?kind=output")
echo "[verify-co-create] file detail ok"

# 6. 版本列表
VERSIONS=$(curl -sfL "${HEADERS[@]}" "${API_BASE}/projects/${PROJECT_ID}/outputs/${FILE_ID}/versions")
