...
function ArtifactIconTile({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`flex w-[5.5rem] shrink-0 flex-col items-center gap-1.5 rounded-xl border bg-gradient-to-b from-slate-900 to-slate-950 px-2 py-3 shadow-inner transition ${
        active
          ? "border-blue-500/60 ring-1 ring-blue-500/40"
          : "border-slate-600/80 hover:border-blue-500/40 hover:ring-1 hover:ring-blue-500/20"
      }`}
      aria-label={`查看 ${label}`}
      aria-pressed={active}
    >
      <FormatIcon format={artifact.format} className="h-9 w-9" />
      <span className="line-clamp-2 w-full text-center text-[10px] leading-tight text-slate-300">
        {label}
      </span>
      <span className="rounded border border-slate-700/80 px-1.5 py-0.5 text-[9px] text-slate-500">
        {formatLabel}
      </span>
    </button>
  );
    ...
    <>
      <div className="mb-3 rounded-2xl border border-slate-700/90 bg-slate-900/70 p-3">
        ...
        {readyArtifacts.length === 0 ? (
          <div className="mt-2 flex min-h-[6.5rem] flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-slate-600/50 bg-slate-950/40 px-4 py-6">
          ...
          <button
            ...
            className="mt-2 flex min-h-[6.5rem] w-full flex-col items-center justify-center gap-2 rounded-xl border border-slate-600/80 bg-gradient-to-b from-slate-900 to-slate-950 px-4 py-5 shadow-inner transition hover:border-blue-500/50 hover:ring-1 hover:ring-blue-500/30"
            aria-label="查看产出物详情"
          >
            ...
            <p className="text-[11px] text-blue-300/80">点击查看详情 ↓</p>
          </button>
        ) : (
      ...
      >
        ...
        {errorMsg && !selectedArtifact && <p className="text-sm text-red-400">❌ {errorMsg}</p>}
        {selectedArtifact
