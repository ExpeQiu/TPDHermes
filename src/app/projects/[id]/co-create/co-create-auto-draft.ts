...
const DOC_NOUN_RE =
  /文稿|文档|稿件|报告|方案|文章|PRD|需求文档|汇报|总结|脚本|纪要|提案|计划|演讲稿|讲话稿|发言稿|讲稿|发布会稿|新闻稿|通稿|主持稿|稿/i;
...
export function normalizeAutoCreateDraftContent(content: string): string {
  let next = stripFileActionsBlock(content);
...
function inferDraftTitleFromPrompt(prompt: string): string | null {
  ...
  const explicit = compact.match(
    /(?:生成|创建|新建|起草|撰写|写)(?:一篇|一个|一份|篇|份|个)?(?:新)?(.{2,48}?(?:演讲稿|讲话稿|发言稿|讲稿|发布会稿|新闻稿|通稿|主持稿|文稿|文档|稿件|报告|方案|文章|PRD|需求文档|汇报|总结|脚本|纪要|提案|计划|稿))/i,
  );
...
export function isDocumentGenerationPrompt(prompt: string): boolean {
  ...
  return /(?:撰写|写|起草|生成).{0,32}稿/.test(normalized.replace(/\s+/g, ""));
}
